...
retry_counter = 0
error = ''
def getFinalVideoUrl(series_id,season_id,episode_id,silent=False,start=True):
	...
	try:
		...
		if prms.has_key("error"):
			...
			if len(error) > 0 :
				if not silent:
					retry(series_id,season_id,episode_id,silent)
				return None,None
...
def retry(series_id,season_id,episode_id,silent):
	global retry_counter
	global error
	
	if __settings__.getSetting('RETRY') != '' and __settings__.getSetting('RETRY') == "true":
		if retry_counter < int(__settings__.getSetting('retry_amount')):
			retry_counter = retry_counter+1
			time.sleep(int(__settings__.getSetting('retry_timer')))
			return getFinalVideoUrl(series_id,season_id,episode_id,silent,False)
		if retry_counter == int(__settings__.getSetting('retry_amount')):
			xbmcgui.Dialog().ok('Error occurred',error)
	else:
		xbmcgui.Dialog().ok('Error occurred',error)
